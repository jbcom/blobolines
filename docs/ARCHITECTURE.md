...
| `src/audio` | ✓ | Howler.js engine playing the itch.io sample library (config/audio.json); music/ambient/sfx channels. The climb's music + ambient bed both follow the canonical biome bands via `biomeBandAt` (`setMusicBand`/`setAmbientBand`, per-band `bandMusic`); the 100m-milestone fanfare escalates by altitude through the pure `milestoneTierIndex(height)` — the SINGLE threshold source that both the audio stinger (`milestoneTierFor`) and the HUD's milestone-banner visual (`milestoneVisual`) key off, so they can't drift apart |
...
| `app/views` | ✓ | `<LandingPage>` (the menu as its OWN pure-DOM page — owns the purple `--bg` + DOM hero blob + `<TitleScreen>`, mounts NO canvas; `app/Game.tsx` renders it on `phase==="menu"`), `<HudOverlay>` (in-run DOM overlay, including next-pad radar, difficulty meter, and high-altitude hazard readout), `<GameOver>` (recap/share/replay plus the achievement-driven next-climb goal), `<DevHarness>` (dev-only, Game.tsx top level), modals |
