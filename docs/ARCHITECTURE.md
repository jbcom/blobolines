...
| `app/views` | ✓ | `<LandingPage>` (the menu as its OWN pure-DOM page — owns the purple `--bg` + DOM hero blob + `<TitleScreen>`, mounts NO canvas; `app/Game.tsx` renders it on `phase==="menu"`), `<HudOverlay>` (in-run DOM overlay, including next-pad radar, difficulty meter, and high-altitude hazard readout), `<GameOver>`, `<DevHarness>` (dev-only, Game.tsx top level), modals |
